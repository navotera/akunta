...
## Locked 2026-08-18 — independent Intern/Fiskal books and fiscal corrections

Akunta supports an entity-level `bookkeeping_mode` with `independent_books` as the dual-book option. In this mode, Intern and Fiskal are complete, independent ledgers. Normal input targets exactly one book; the explicit `Intern & Fiskal` input option atomically creates two draft journals with the same transaction code and an `input_group_id` link, while retaining separate numbers, entries, posting state, and balances. Account availability (`intern`, `fiskal`, `both`) controls selectable accounts only and never synchronizes balances.
...
Account descriptions are stored on `accounts`, exposed by the tenant-scoped API, editable in the COA form, searchable, and available as tooltips in list/tree/T-view. A curated template description contains three practical elements: definition, when to use the account, and a representative economic event. It is not generated at runtime from an account name.

Technology & IT template accounts all carry explicit curated descriptions. Fake-data re-import may backfill those descriptions only when the account itself is proven fake by an entity-scoped marker. It never overwrites a manual account that merely has the same code.

## Locked 2026-08-18 — Fake accounts become permanent through real use

The account API exposes entity-scoped fake provenance for UI indicators. A grey dot means the account is still owned by the removable demo dataset. Creating or changing a non-fake journal entry to use that account removes the account marker and all ancestor-account markers in the same database transaction. Entries generated by the fake-data importer are explicitly excluded and do not promote their accounts.

Technology COA import resolves duplicates conservatively. Exact or normalized codes take priority; name matching requires compatible account type, normal balance, postable status, and book availability, followed by exact normalized or very-high-similarity names. A compatible manual account can satisfy a canonical demo role without being renamed, overwritten, or marked fake. The Technology & IT template also covers implementation/integration, support and maintenance, cybersecurity/audit, IT training, direct customer support, third-party service licenses, product research, and internal security/compliance.

## Locked 2026-08-18 — Account SOP backfill and availability review
