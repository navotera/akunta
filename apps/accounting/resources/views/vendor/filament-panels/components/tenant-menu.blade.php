    ...
    @if ($canSwitchTenants)
        <x-filament::dropdown.list>
            @foreach ($tenants as $tenant)
                <x-filament::dropdown.list.item
                    {{-- Akunta override: route tenant switch to Dashboard, not first nav item.
                         Default `filament()->getUrl($tenant)` falls back to first registered nav
                         (EcopaProfile), since Dashboard nav is intentionally unregistered. --}}
                    :href="\App\Filament\Pages\Dashboard::getUrl(panel: 'accounting', tenant: $tenant)"
