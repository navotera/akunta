<script lang="ts">
  ...
  const cashflowIncome = [120, 140, 165, 150, 178, 195, 210, 198, 220, 245, 260, 285].map((v) => v * 1e6);
  const cashflowExpense = [80, 92, 100, 95, 110, 105, 115, 108, 120, 118, 125, 130].map((v) => v * 1e6);
  const cashflowLabels = Array.from({ length: 12 }, (_, i) => `W${i + 1}`);
  const cashflowNet = cashflowIncome.reduce((a, b) => a + b, 0) - cashflowExpense.reduce((a, b) => a + b, 0);

  const donutSlices = [
    { value: 58, color: '#1B84FF', label: 'Penjualan POS', amount: 108180000, src: 'App Penjualan' },
  ...
  const aging = [
    { label: 'Piutang Usaha', sub: '42 invoice', cells: [68420000, 24180000, 8420000, 3120000], accent: '#1B84FF' },
    { label: 'Utang Usaha',   sub: '18 tagihan', cells: [42100000, 18650000, 0, 0],            accent: '#F6C000' },
    { label: 'Pajak Terutang',sub: 'PPN Mei',   cells: [12480000, 0, 0, 0],                   accent: '#0284C7' },
  ];

  const approvals = [
    { icon: '🧾', title: 'Invoice INV-2056', sub: 'Toko Sumber Rejeki · Rp 18.420.000', tag: 'App Invoice', warn: false },
    { icon: '🛒', title: 'PO #PO-1183', sub: 'CV Anugerah Sentosa · Rp 7.250.000', tag: 'App Pembelian', warn: false },
    { icon: '👥', title: 'Penyesuaian Gaji Mei', sub: '3 karyawan · Rp 2.850.000', tag: 'App Payroll', warn: true },
    { icon: '🧾', title: 'Refund INV-2031', sub: 'Bunga Mawar Cake · Rp 480.000', tag: 'App Invoice', warn: false },
    { icon: '📒', title: 'Jurnal Penyesuaian', sub: 'Reklas biaya sewa Q2', tag: 'Manual', warn: false },
  ];
  ...
  const ecoSync = [
    { icon: '📈', name: 'App Penjualan', last: '2 menit lalu', status: 'ok' as const, count: '142 transaksi' },
    { icon: '🛒', name: 'App Pembelian', last: '6 menit lalu', status: 'ok' as const, count: '38 dokumen' },
    { icon: '📦', name: 'App Inventory', last: '14 menit lalu', status: 'ok' as const, count: '11 mutasi' },
    { icon: '🧾', name: 'App Invoice', last: '3 menit lalu', status: 'ok' as const, count: '27 invoice' },
    { icon: '👥', name: 'App Payroll', last: 'kemarin 17:02', status: 'warn' as const, count: '1 selisih' },
    { icon: '📑', name: 'App e-Faktur', last: 'sedang sinkron', status: 'syncing' as const, count: '—' },
  ];
...
<div class="px-6 py-5">
  ...
  {:else if pulse}
    ...
    <section class="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4 mb-4">
      {#each [
        { label: 'Saldo Kas & Bank', value: 428750000, delta: '+8,4%', pos: true,  data: [40,42,44,41,46,48,52,54,57,60,63,68], hint: '6 rekening' },
        { label: 'Pendapatan (MTD)', value: Number(pulse.revenue.current), delta: pctDelta(pulse.revenue.current, pulse.revenue.previous)?.value ?? '—', pos: pctDelta(pulse.revenue.current, pulse.revenue.previous)?.positive ?? true, data: [20,22,24,28,30,33,38,42,46,50,55,60], hint: 'vs bulan lalu' },
        { label: 'Beban (MTD)',      value: Number(pulse.expenses.current), delta: pctDelta(pulse.expenses.current, pulse.expenses.previous)?.value ?? '—', pos: pctDelta(pulse.expenses.current, pulse.expenses.previous)?.positive ?? true, data: [35,38,40,42,44,42,40,38,36,35,33,32], hint: 'vs bulan lalu' },
        { label: 'Laba Bersih (MTD)',value: Number(pulse.net_income.current), delta: pctDelta(pulse.net_income.current, pulse.net_income.previous)?.value ?? '—', pos: Number(pulse.net_income.current) >= 0, data: [10,12,14,18,20,24,28,32,36,40,44,48], hint: pulse.period_label },
      ] as kpi (kpi.label)}
          ...
          <div class="flex items-end justify-between gap-2">
            <div>
              <div class="text-[1.4rem] font-bold tabnum leading-none tracking-tight">
                <span class="text-xs text-text-muted font-medium mr-1">Rp</span>{compact(kpi.value)}
              </div>
      ...
      <div class="rounded-lg border border-border-default bg-card-bg p-4 shadow-xs">
        <div class="flex items-baseline justify-between mb-3">
          <div>
            ...
            <div class="text-[0.7rem] text-text-muted mt-0.5">{pulse.period_label} · per minggu</div>
          </div>
          <div class="inline-flex gap-0.5 bg-page-bg p-0.5 rounded-md border border-border-default text-xs">
            {#each ['7H','30H','90H','YTD'] as t, i (t)}
              <button
                type="button"
                class="px-3 py-1 rounded font-medium {cashflowTab === i ? 'bg-card-bg text-text-default shadow-xs' : 'text-text-muted hover:text-text-default'}"
        ...
        <div class="flex flex-wrap items-center gap-4 mb-2 text-xs">
          <span class="inline-flex items-center gap-1.5">
            ...
            <span>Pemasukan</span>
            <span class="tabnum text-text-muted">· Rp {compact(cashflowIncome.reduce((a,b)=>a+b,0))}</span>
          </span>
          <span class="inline-flex items-center gap-1.5">
            <svg width="14" height="6"><line x1="0" y1="3" x2="14" y2="3" stroke="currentColor" class="text-text-muted" stroke-width="1.6" stroke-dasharray="3 3"/></svg>
            <span>Pengeluaran</span>
            <span class="tabnum text-text-muted">· Rp {compact(cashflowExpense.reduce((a,b)=>a+b,0))}</span>
          </span>
          <span class="ml-auto text-text-muted">
            Net <strong class="tabnum text-paid font-semibold">+ Rp {compact(cashflowNet)}</strong>
          </span>
        </div>
        <CashflowChart income={cashflowIncome} expense={cashflowExpense} labels={cashflowLabels} />
      </div>
          ...
          <div class="relative shrink-0">
            ...
            <div class="absolute inset-0 grid place-items-center text-center">
              <div>
                ...
                <div class="tabnum text-[0.95rem] font-semibold">Rp {compact(donutSlices.reduce((s,d)=>s+d.amount,0))}</div>
              </div>
            </div>
          </div>
    ...
    <section class="grid grid-cols-1 gap-3 lg:grid-cols-[1.4fr_1fr] mb-4">
      <div class="rounded-lg border border-border-default bg-card-bg shadow-xs overflow-hidden">
        <div class="flex items-center justify-between border-b border-border-default px-4 py-3">
          ...
          <div class="inline-flex gap-0.5 bg-page-bg p-0.5 rounded-md border border-border-default text-xs">
            <button class="px-3 py-1 rounded font-medium bg-card-bg text-text-default shadow-xs">Ringkasan</button>
              ...
              <tr class="border-t border-border-soft hover:bg-page-bg cursor-pointer">
                <td class="px-4 py-2.5">
                  <div class="flex items-center gap-2.5">
                    <span class="w-[3px] h-6 rounded-sm shrink-0" style="background: {r.accent}"></span>
                    <div>
                ...
                {#each r.cells as c, i (i)}
                  <td class="px-4 py-2.5 tabnum text-xs {c === 0 ? 'text-text-muted/60' : i >= 2 ? 'text-danger' : ''}">
                    {c === 0 ? '—' : 'Rp ' + compact(c)}
                  </td>
                {/each}
                <td class="px-4 py-2.5 text-right tabnum text-xs font-semibold">Rp {compact(total)}</td>
              </tr>
            {/each}
          </tbody>
        </table>
      ...
      <div class="rounded-lg border border-border-default bg-card-bg p-4 shadow-xs">
        ...
        <div class="flex flex-col gap-2">
          {#each approvals as a (a.title)}
            <div class="flex items-center gap-2.5 p-2 border border-border-default rounded-md bg-card-bg">
              <div
                class="w-7 h-7 rounded-md grid place-items-center text-sm shrink-0"
                style="background: {a.warn ? 'var(--m-warning-light, #FFF8DD)' : 'var(--m-primary-light)'}; color: {a.warn ? '#F6C000' : '#1B84FF'};"
              >
                {a.icon}
              </div>
              <div class="flex-1 min-w-0">
                <div class="text-xs font-semibold truncate">{a.title}</div>
                <div class="text-[0.65rem] text-text-muted truncate">{a.sub} · <span>{a.tag}</span></div>
              </div>
              <button class="w-6 h-6 grid place-items-center rounded text-text-muted hover:text-danger hover:bg-danger-light" title="Tolak">×</button>
              <button class="w-6 h-6 grid place-items-center rounded bg-primary text-white text-xs hover:bg-primary-active" title="Setujui">✓</button>
            </div>
          {/each}
        </div>
        <button class="w-full mt-2.5 py-2 text-xs font-medium text-text-muted hover:text-text-default rounded border border-border-default hover:bg-page-bg">
          Lihat semua (12) ›
        </button>
      </div>
    </section>
    ...
    <section class="grid grid-cols-1 gap-3 lg:grid-cols-2">
      <div class="rounded-lg border border-border-default bg-card-bg p-4 shadow-xs">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {#each quickActions as q (q.label)}
            ...
            >
              <div class="w-7 h-7 rounded-md grid place-items-center bg-primary-light text-primary text-sm">{q.icon}</div>
      ...
      <div class="rounded-lg border border-border-default bg-card-bg p-4 shadow-xs">
        <div class="flex items-baseline justify-between mb-3">
          ...
          <button class="text-[0.7rem] font-medium text-text-muted hover:text-primary">↻ Sync sekarang</button>
        </div>
        <div class="flex flex-col">
          {#each ecoSync as e (e.name)}
            {@const p = statusPill(e.status)}
            <div class="flex items-center gap-2.5 py-2 border-b border-border-soft last:border-b-0">
              <div class="w-7 h-7 rounded-md grid place-items-center bg-page-bg text-sm shrink-0">{e.icon}</div>
              <div class="flex-1 min-w-0">
              ...
              <span class="ak-pill" style="background: {p.bg}; color: {p.fg}; gap: 4px;">
                <span class="w-1.5 h-1.5 rounded-full" style="background: {p.dot}; box-shadow: {e.status === 'syncing' ? '0 0 0 3px ' + p.bg : 'none'}"></span>
                {p.label}
              </span>
            </div>
          {/each}
    ...
    {#if recent.length}
      <section class="mt-3 rounded-lg border border-border-default bg-card-bg shadow-xs">
        <header class="flex items-center justify-between border-b border-border-soft px-4 py-3">
          <strong class="text-[0.7rem] font-bold uppercase tracking-wider text-text-muted">Jurnal Terakhir</strong>
          ...
          <tbody>
            {#each recent as j (j.id)}
              <tr class="border-t border-border-soft hover:bg-page-bg cursor-pointer" onclick={() => goto(`/journals/${j.id}`)}>
                ...
                <td class="px-4 py-2 text-xs">{j.date}</td>
                <td class="px-4 py-2 text-xs">{j.memo ?? '—'}</td>
                <td class="px-4 py-2 text-right font-mono tabnum text-xs">{formatRupiah(j.total)}</td>
                <td class="px-4 py-2">
                  <span class="ak-pill {j.status === 'posted' ? 'bg-paid-light text-paid' : j.status === 'reversed' ? 'bg-warning-light text-warning' : 'bg-page-bg text-text-muted'}">
                    {j.status}
                  </span>
                </td>
              </tr>
