<script lang="ts">
  ...
  import { webhookApi, type WebhookSubscription, type WebhookCreateInput, type WebhookDeliveryLog } from '$lib/api/webhook.js';
  ...
  async function loadDeliveryLogs() {
    ...
    try { deliveryLogs = (await webhookApi.logs()).data; }
    catch (e) { logsError = e instanceof ApiError ? `Server ${e.status}` : (e as Error).message; }
  ...
  let webhookForm = $state<WebhookCreateInput>({ event: 'journal.posted', description: '', is_active: true });
  ...
  function logStatus(log: WebhookDeliveryLog): { label: string; tone: string } {
    ...
    if (log.status === 'pending') return { label: 'Menunggu', tone: 'bg-warning-light text-warning' };
    return { label: log.status === 'giving_up' ? 'Dihentikan' : 'Gagal', tone: 'bg-danger-light text-danger' };
  }
...
<div class="px-6 py-6">
  <header class="mb-5 flex items-center justify-between">
    <div>
      <div class="flex items-center gap-1 text-xs font-medium text-text-muted">
        <button type="button" class="hover:text-primary hover:underline" onclick={() => goto('/journals')}>Jurnal</button>
        <span aria-hidden="true">›</span>
        <button type="button" class="hover:text-primary hover:underline" onclick={() => goto('/auto-mapping')}>Auto Mapping</button>
        <span aria-hidden="true">›</span>
  ...
  <div class="grid grid-cols-1 gap-5 lg:grid-cols-[210px_minmax(0,1fr)]">
    <aside class="h-fit rounded-lg border border-border-default bg-card-bg p-2 shadow-xs">
      <p class="px-3 pb-2 pt-2 text-[0.68rem] font-semibold uppercase tracking-wider text-text-muted">Integrasi</p>
      <nav class="space-y-1" aria-label="Menu integrasi">
        <button
          type="button"
          class="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm font-medium transition {activeSection === 'webhooks' ? 'bg-primary-light text-primary' : 'text-text-default hover:bg-page-bg'}"
        ...
        <button
          type="button"
          class="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm font-medium transition {activeSection === 'logs' ? 'bg-primary-light text-primary' : 'text-text-default hover:bg-page-bg'}"
        ...
        <button
          type="button"
          class="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm font-medium transition {activeSection === 'docs' ? 'bg-primary-light text-primary' : 'text-text-default hover:bg-page-bg'}"
  ...
  <section class="mt-8">
    <header class="mb-3 flex items-center justify-between">
      <div>
        ...
        <p class="text-xs text-text-muted">Buat URL webhook untuk menerima payload dari aplikasi eksternal.</p>
      </div>
      <button
        type="button"
        class="rounded-md bg-primary px-3 py-1.5 text-sm font-semibold text-white hover:bg-primary-active"
              ...
              >
                ...
                <td class="px-4 py-2 text-right">
                  <a
                    ...
                    onclick={(event) => { event.preventDefault(); copyWebhook(w.url); }}
                  >Copy webhook</a>
                  <button type="button" class="text-xs text-primary hover:underline mr-3" onclick={() => regenerateWebhookUrl(w)}>Buat URL baru</button>
                ...
                <tr class="border-t border-border-soft bg-page-bg">
                  <td colspan="6" class="px-5 py-4">
                    <div class="mb-2 flex items-center justify-between">
                      <p class="text-xs font-semibold uppercase tracking-wider text-text-muted">Log koneksi webhook</p>
                    ...
                    {:else if selectedWebhookLogs.length === 0}
                      <p class="text-sm text-text-muted">Belum ada log koneksi untuk webhook ini.</p>
                    {:else}
                      <div class="overflow-x-auto rounded-md border border-border-default bg-card-bg">
                        <table class="w-full text-xs">
                          <thead class="bg-page-bg text-left uppercase tracking-wider text-text-muted">
                            <tr><th class="px-3 py-2">Waktu</th><th class="px-3 py-2">Event</th><th class="px-3 py-2">Status</th><th class="px-3 py-2">Response</th></tr>
                          </thead>
                          <tbody>
                            {#each selectedWebhookLogs as log (log.id)}
                              {@const detailStatus = logStatus(log)}
                              <tr class="border-t border-border-soft">
                                <td class="px-3 py-2 text-text-muted">{fmtDate(log.created_at)}</td>
                                <td class="px-3 py-2 font-mono">{log.event}</td>
                                <td class="px-3 py-2"><span class="rounded-full px-2 py-1 font-semibold {detailStatus.tone}">{detailStatus.label}</span></td>
    ...
    {:else if activeSection === 'logs'}
      <section class="rounded-lg border border-border-default bg-card-bg shadow-xs">
        <header class="flex flex-wrap items-start justify-between gap-3 border-b border-border-soft px-5 py-4">
          <div>
            <h2 class="text-base font-semibold">Log history webhook</h2>
            <p class="mt-1 text-xs text-text-muted">Riwayat pengiriman dari setiap aplikasi yang menggunakan webhook. Log disimpan selama 12 bulan.</p>
          </div>
          <button
            type="button"
        ...
        {:else if logsError}
          <div class="m-5 rounded-md border border-danger bg-danger-light p-3 text-sm text-danger">{logsError}</div>
        {:else if deliveryLogs.length === 0}
          <div class="p-8 text-center text-sm text-text-muted">Belum ada log pengiriman webhook dalam 12 bulan terakhir.</div>
        {:else}
              ...
              <tbody>
                {#each deliveryLogs as log (log.id)}
                  ...
                  <tr class="border-t border-border-soft align-top">
                    <td class="whitespace-nowrap px-5 py-3 text-xs text-text-muted">{fmtDate(log.created_at)}</td>
                    <td class="px-3 py-3 font-medium">{log.description || '—'}</td>
                    <td class="px-3 py-3">
                      <div class="font-mono text-xs">{log.event}</div>
                      {#if log.url}<div class="mt-1 max-w-[220px] truncate text-[11px] text-text-muted" title={log.url}>{log.url}</div>{/if}
                      {#if log.error}<div class="mt-1 max-w-[260px] text-xs text-danger" title={log.error}>{log.error}</div>{/if}
                    </td>
                    <td class="px-3 py-3"><span class="rounded-full px-2 py-1 text-[11px] font-semibold {status.tone}">{status.label}</span></td>
    ...
    {:else}
      <section class="rounded-lg border border-border-default bg-card-bg p-5 shadow-xs">
        ...
        <p class="mt-1 text-sm text-text-muted">Kirim data JSON dari aplikasi lain ke Akunta dalam tiga langkah.</p>

        <div class="mt-5 grid gap-3 md:grid-cols-3">
          <div class="rounded-md border border-border-default p-4">
            <span class="mb-3 flex h-7 w-7 items-center justify-center rounded-full bg-primary-light text-xs font-bold text-primary">1</span>
            <h3 class="font-semibold text-text-default">Buat webhook</h3>
            <p class="mt-1 text-sm leading-6 text-text-muted">Klik <strong>+ New</strong>, pilih event, lalu simpan.</p>
          </div>
          <div class="rounded-md border border-border-default p-4">
            <span class="mb-3 flex h-7 w-7 items-center justify-center rounded-full bg-primary-light text-xs font-bold text-primary">2</span>
            <h3 class="font-semibold text-text-default">Salin URL</h3>
            <p class="mt-1 text-sm leading-6 text-text-muted">Gunakan URL yang muncul sebagai tujuan request dari aplikasi Anda.</p>
          </div>
          <div class="rounded-md border border-border-default p-4">
            <span class="mb-3 flex h-7 w-7 items-center justify-center rounded-full bg-primary-light text-xs font-bold text-primary">3</span>
            <h3 class="font-semibold text-text-default">Kirim JSON</h3>
            <p class="mt-1 text-sm leading-6 text-text-muted">Kirim data dengan metode <code>POST</code> dan <code>Content-Type: application/json</code>.</p>
          </div>
        </div>
  ...
  -d '&#123;"reference":"INV-001","amount":125000&#125;'</pre>
          ...
          <p class="mt-2 text-xs text-text-muted">Salin URL lengkap dari modal. Secret berada di bagian terakhir URL, bukan dikirim sebagai parameter atau header terpisah. Header <code>X-Akunta-Event</code> opsional.</p>
        </div>
        ...
        <div class="mt-5 grid gap-3 md:grid-cols-2">
          <div class="rounded-md border border-paid/30 bg-paid-light p-4 text-sm">
            ...
            <p class="mt-1 text-text-muted">Akunta membalas status <code>202</code>. Hasil request dapat dilihat di menu <strong>Log history</strong>.</p>
          </div>
          <div class="rounded-md border border-warning/30 bg-warning-light p-4 text-sm">
            ...
            <p class="mt-1 text-text-muted">Bagian <code>/SECRET_ANDA</code> adalah kunci akses webhook. Jika URL bocor, gunakan <strong>Buat URL baru</strong>; URL lama langsung tidak berlaku.</p>
          </div>
        </div>
      </section>
    {/if}
  ...
  >
    ...
    >
      {#if revealedWebhook}
        ...
        <p class="mb-2 text-sm text-text-muted">Salin URL lengkap ini dan gunakan sebagai endpoint POST di aplikasi pengirim.</p>
      ...
      {:else}
        ...
        <p class="mb-4 text-sm text-text-muted">Pilih event yang boleh diterima melalui URL webhook yang dibuat Akunta.</p>
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
          <div>
            <p class="mb-2 text-xs font-semibold uppercase tracking-wider text-text-muted">Event list</p>
            <div class="flex flex-wrap gap-2">
              {#each WEBHOOK_EVENTS as event}
                <button
                  type="button"
                  class="rounded-full border px-3 py-1.5 font-mono text-xs font-semibold transition {webhookForm.event === event ? 'border-primary bg-primary text-white' : 'border-border-default bg-card-bg text-text-muted hover:border-primary hover:text-primary'}"
            ...
            {#if fieldErr('event')}<span class="mt-2 block text-xs text-danger">{fieldErr('event')}</span>{/if}
          </div>
          <div class="rounded-md border border-border-default bg-page-bg p-4">
            <p class="mb-2 text-xs font-semibold uppercase tracking-wider text-text-muted">Informasi event</p>
            <p class="font-mono text-sm font-semibold text-text-default">{webhookForm.event}</p>
            <p class="mt-2 text-sm leading-6 text-text-muted">{WEBHOOK_EVENT_INFO[webhookForm.event] ?? 'Event webhook dari Akunta.'}</p>
          </div>
        </div>
