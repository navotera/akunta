<script lang="ts">
  ...
  let noAttachment = $state(restoredDraft?.attachments_not_available ?? false);
  ...
  const isBlankRow = (row: Row) =>
    !row.account_id && !row.memo?.trim() && Number(row.amount || 0) === 0;
  if (debits.length > 1 && debits.every(isBlankRow)) debits = debits.slice(0, 1);
  ...
  $effect(() => {
    ...
    const draft: JournalDraft = {
      ...
      memo,
      reference,
      attachments_not_available: noAttachment,
  ...
  function toggleNoAttachment(event: Event) {
    noAttachment = (event.currentTarget as HTMLInputElement).checked;
    attachmentError = null;
    if (noAttachment) attachments = [];
  }

  function validateAttachments(): boolean {
    if (templateMode || initial || noAttachment || attachments.length > 0) {
      attachmentError = null;
      return true;
    }

    attachmentError = 'Lampiran wajib diisi atau centang “Lampiran tidak ada”.';
    return false;
  }
  ...
  async function handlePickTemplate(t: JournalTemplateSummary) {
    ...
    try {
      ...
      debits = dRows.length ? dRows : [blankRow()];
...
<div class="ak-journal-shell">
  <fieldset disabled={readOnly} class="contents">
        ...
        <div>
          <div class="flex flex-wrap items-center gap-2">
            ...
            {#if initial?.status === 'draft'}
              <span
                class="inline-flex items-center rounded-full bg-[#f1df9a] px-2 py-0.5 text-xs font-semibold text-[#b38c00]"
    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-4 px-6 pb-32">
      {#if attachmentError || serverMessage || fieldError('number') || fieldError('entries') || fieldError('tenant')}
        <div
        ...
        <section class="rounded-lg border border-border-default bg-card-bg p-4 shadow-xs">
          <div class="flex flex-col gap-3">
            {#if !templateMode}
              <label class="text-sm">
                ...
                {#if fieldError('transaction_code')}
                  <span class="block mt-1 text-xs text-danger"
                    >{fieldError('transaction_code')}</span
                  >
                {/if}
              </label>
              <label class="text-sm">
            ...
            {#if !templateMode}
              <label class="text-sm">
                <span class="block font-medium mb-1"
                  >Tanggal <span class="text-danger">*</span></span
                >
                <DateInput
                  value={date}
  ...
  >
    ...
    <div class="flex items-center gap-3">
      {#if !readOnly && !templateMode && (allowPosting || initial)}
        <button
          type="button"
      ...
      {/if}
      {#if !readOnly && !templateMode && reviewMode}
        <button
          type="button"
      ...
      {:else if !readOnly && !templateMode && allowPosting}
        <button
          type="button"
      ...
      {/if}
      {#if readOnly && initial?.status === 'submitted' && onCancelReview}
        <button
          type="button"
