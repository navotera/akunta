...
>
  <header class="mb-3">
    ...
    <span class="block text-xs text-text-muted leading-snug mt-1"
      >Klik untuk duplikat sebagai dasar</span
    >
  </header>
  ...
  {#if recent}
    ...
    >
      <div
        class="flex items-baseline justify-between text-xs text-text-muted font-mono tabnum mb-1"
      >
        <span>{recent.number}</span>
        <span>{formatDate(recent.date)}</span>
      </div>
      <strong class="block truncate text-sm font-bold text-text-default"
        >{recent.memo ?? 'Tanpa keterangan'}</strong
      >
      <small class="mt-1 block text-xs text-text-muted font-mono tabnum"
        >{formatRupiah(recent.total)}</small
      >
    </a>
  {:else}
